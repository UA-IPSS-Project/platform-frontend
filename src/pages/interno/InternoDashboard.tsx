...
export function InternoDashboard({ isDarkMode, onToggleDarkMode, onLogout }: InternoDashboardProps) {
    ...
    const renderContent = () => {
        ...
        return (
            <div className="flex items-center justify-center h-[500px]">
                <div className="text-center">
                    <h2 className="text-2xl text-muted-foreground mb-2">Painel Interno</h2>
                    <p className="text-muted-foreground">Em desenvolvimento</p>
                </div>
            </div>
        );
    ...
    const navigationContent = (
        <>
            <Button
                ...
                className={`text-sm ${currentView === 'home' ? 'bg-primary hover:bg-primary/90 text-primary-foreground' : 'text-foreground hover:bg-primary/10 hover:text-primary'}`}
            >
                {t('sidebar.home')}
            </Button>
            <NavDropdown
